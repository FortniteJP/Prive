...
public abstract class UNetConnection : UPlayer {
    ...
    private float _LastReceiveRealTime;
    private float _LastGoodPacketRealtime;
    ...
    public virtual void Tick(float deltaSeconds) {
        ...
        for (var i = _ChannelsToTick.Count - 1; i >= 0; i--) {
            ...
            if (_ChannelsToTick[i].CanStopTicking()) _ChannelsToTick.RemoveAt(i);
        }
        ...
        if (TimeSensitive || (Driver!.GetElapsedTime() - _LastSendTime) > Driver.KeepAliveTime) {
            ...
            if (bHandlerHandshakeComplete) FlushNet();
        }
    ...
    public void FlushPacketOrderCache(bool bFlushWholeCache = false) {
        if (_PacketOrderCache is not null && _PacketOrderCacheCount > 0) {
            var cache = _PacketOrderCache;
            var cacheEndIdx = _PacketOrderCacheStartIdx - 1; // PacketOrderCache->GetPreviousIndex(PacketOrderCacheStartIdx);
            bool bEndOfCacheSet = cache!.Count == cacheEndIdx;

            _bFlushingPacketOrderCache = true;

            bFlushWholeCache = bFlushWholeCache || bEndOfCacheSet;

            while (_PacketOrderCacheCount > 0) {
                var curCachePacket = cache[_PacketOrderCacheStartIdx];

                if (curCachePacket is not null) {
                    Console.WriteLine($"'Out of Order' packet cache, replaying packet with cache index: {_PacketOrderCacheStartIdx} (bFlushWholeCache: {bFlushWholeCache})");
                    ReceivedPacket(curCachePacket);
                    curCachePacket.Reset();
                    _PacketOrderCacheCount--;
                } else if (!bFlushWholeCache) {
                    break;
                }
                _PacketOrderCacheStartIdx += 1; // PacketOrderCache->GetNextIndex(PacketOrderCacheStartIdx);
            }
    ...
    public virtual void ReceivedRawPacket(FReceivedPacketView packetView) {
        ...
        if (packetView.DataView.NumBytes() > 0) {
            ...
            if (lastByte != 0) {
                ...
                if (Handler != null) Handler.IncomingHigh(reader);
    ...
    private void ReceivedPacket(FBitReader reader, bool bIsReinjectedPacket = false) {
        ...
        while (!reader.AtEnd() && State != EConnectionState.USOCK_Closed) {
            if (_bInternalAck && EngineNetworkProtocolVersion < EEngineNetworkVersionHistory.HISTORY_ACKS_INCLUDED_IN_HEADER) _ = reader.ReadBit();
            ...
            // Process Received data
            {
                ...
                if (bunch.bReliable) {
                    ...
                    else bunch.ChSequence = MakeRelative((int)reader.ReadInt(MaxChSequence), InReliable[bunch.ChIndex], MaxChSequence);
                }
                ...
                if (bunch.EngineNetVer() < EEngineNetworkVersionHistory.HISTORY_CHANNEL_NAMES) {
                    ...
                    if (bunch.bReliable || bunch.bOpen) {
                        ...
                        if (bunch.ChName == EName.Control) bunch.ChType = EChannelType.CHTYPE_Control;
                        else if (bunch.ChName == EName.Voice) bunch.ChType = EChannelType.CHTYPE_Voice;
                        else if (bunch.ChName == EName.Actor) bunch.ChType = EChannelType.CHTYPE_Actor;
                    } else {
                ...
                if (channel != null && (bunch.ChName != EName.None) && (bunch.ChName != channel.ChName)) {
                ...
                if (bLocalSkipAck) bSkipAck = true;
        ...
        foreach (var info in channelsToClose) {
            ...
            if (channel != null) channel.ConditionalCleanUp(false, info.CloseReason);
        }

        // TODO: ValidateSendBuffer();

        if (!bSkipAck) _LastGoodPacketRealtime = DateTimeOffset.UtcNow.ToUnixTimeSeconds();

        if (!_bInternalAck) {
            if (bSkipAck) PacketNotify.NakSeq(new SequenceNumber((ushort)InPacketId));
            else {
    ...
    private void PacketNotifyUpdate(PacketNotifyUpdateContext context, SequenceNumber ackedSequence, bool delivered) {
        ...
        if (delivered) ReceivedAck(LastNotifiedPacketId); // List<FChannelCloseInfo> outChannelsToClose
        else ReceivedNak(LastNotifiedPacketId);
    }
    ...
    private void ReceivedAck(int ackPacketId) { // List<FChannelCloseInfo> outChannelsToClose
        ...
        var ackChannelFunc = new Action<int, uint>((ackedPacketId, channelIndex) => {
            if (channelIndex >= Channels.Length) {
                Console.WriteLine($"Received ack for channel {channelIndex} but we only have {Channels.Length} channels");
                return;
            }
            var channel = Channels[channelIndex];
            if (channel != null) {
                if (channel.OpenPacketId.Last == ackedPacketId) channel.OpenAcked = true;
                ...
                for (var outBunch = channel.OutRec; outBunch != null; outBunch = outBunch.Next) {
                    ...
                    if (outBunch.PacketId == ackedPacketId) outBunch.ReceivedAck = true;
                }
    ...
    public void InitSequence(int incomingSequence, int outgoingSequence) {
        if (InPacketId == -1) {
            ...
            for (var i = 0; i < InReliable.Length; i++) InReliable[i] = InitInReliable;
    ...
    private void InitHandler() {
        ...
        if (Driver == null) throw new UnrealNetException("Driver was null");
    ...
    public long GetMaxSingleBunchSizeBits() => (MaxPacket * 8) - MaxBunchHeaderBits - MaxPacketTrailerBits - MaxPacketHeaderBits - MaxPacketHandlerBits;

    public UChannel CreateChannelByName(FName chName, EChannelCreateFlags createFlags, int chIndex) {
        if (!Driver!.IsKnownChannelName(chName)) throw new UnrealNetException("Unknown channel name was specified.");
        ...
        if (chIndex >= Channels.Length) throw new UnrealNetException("Channel index is too high.");

        if (Channels[chIndex] != null) throw new UnrealNetException("Trying to replace an existing channel.");
        
        // Create channel.
        var channel = Driver.GetOrCreateChannelByName(chName);
        if (channel == null) throw new UnrealNetException("Failed to create channel.");
        ...
        if (Driver.ChannelDefinitionMap[chName].TickOnCreate) StartTickingChannel(channel);
    ...
    public int SendRawBunch(FOutBunch bunch, bool inAllowMerge) {
        ...
        if (Driver == null || bunch.ReceivedAck || bunch.IsError()) throw new UnrealNetException();
    ...
    private void PrepareWriteBitsToSendBuffer(long sizeInBits, long extraSizeInBits) {
        ...
        if (totalSizeInBits > GetFreeSendBufferBits()) FlushNet();
    ...
    private void WritePacketHeader(FBitWriter writer) {
        ...
        if (bIsHeaderUpdate) {
            ...
            if (bWroteHeader) HasDirtyAcks = 0;
        }
    }
    ...
    private int WriteBitsToSendBufferInternal(byte[]? bits, int sizeInBits, byte[]? extraBits, int extraSizeInBits, EWriteBitsDataType dataType) {
        ...
        if (extraSizeInBits != 0) {
            if (extraBits == null) throw new UnrealNetException("extraBits should not be null if a size is set");
        ...
        if (dataType == EWriteBitsDataType.Bunch) NumBunchBits += sizeInBits + extraSizeInBits;
        
        // Flush now if we are full
        if (GetFreeSendBufferBits() == 0) FlushNet();
    ...
    public void FlushNet(bool bIgnoreSimulation = false) {
        if (Driver == null) throw new UnrealNetException();
        ...
        if (SendBuffer.GetNumBits() != 0 || HasDirtyAcks != 0 || (Driver.GetElapsedTime() - _LastSendTime > Driver.KeepAliveTime && !IsInternalAck() && State != EConnectionState.USOCK_Closed)) {
            ...
            if (Driver.IsNetResourceValid()) LowLevelSend(SendBuffer.GetData(), (int)SendBuffer.GetNumBits(), traits);
            ...
            OutLagTime[index] = FPlatformTime.Seconds(); // packetSentTimeInS
    ...
    private int GetFreeChannelIndex(FName chName) {
        ...
        if (staticChannelIndex != -1) firstChannel = staticChannelIndex;
                
        // Search the channel array for an available location
        for (chIndex = firstChannel; chIndex < Channels.Length; chIndex++) {
            if (Channels[chIndex] == null) break;
        }

        if (chIndex == Channels.Length) chIndex = UnrealConstants.IndexNone;

        return chIndex;
    }

    private void StartTickingChannel(UChannel channel) => _ChannelsToTick.Add(channel);

    private void StopTickingChannel(UChannel channel) => _ChannelsToTick.Remove(channel);
    ...
    private bool HasReceivedClientPacket() => IsInternalAck() || !Driver!.IsServer() || InReliable[0] != InitInReliable;

    public bool IsInternalAck() => _bInternalAck;
